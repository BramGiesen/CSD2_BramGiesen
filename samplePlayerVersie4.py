...
kansKick =  [10, 10, 10, 10, 10, 10]
kansSnare = [0, 0, 0, 0, 0]
kansHihat = [0, 0, 0, 0, 0, 0, 0]
...
sequence1 = [0, 2, 4, 6]#sequenceKick
sequence2 = [3]#sequenceSnare
sequence3 = [5]#sequenceHihat
def makeList():
  ...
  if(currentTime - startTime >= event[0]):
    ...
    if events:
      ...
      event = events.pop(0)
        ...
        while True:
                currentTime = time.time()#list is empty, wait untill measure is done, then refill
                if currentTime - startTime >= measureDuration:
                    startTime = time.time()#set new time
                    makeList()#generate new list
                    event = events.pop(0)
                    break;
                else:
